...
function meta = parseFilename(fname)
    ...
    % Pattern: VehicleName_YYYY-MM-DD_HH-MM-SS_mode_Event
...
% =========================================================
%% SIGNAL EXTRACTION — Cell-Array of Timetables
%  Based on proven extractTimetableFromCell logic
% =========================================================
function S = extractSignals(Data, Channels)
    ...
    % --- Step 1: Build fast lookup map: signalName -> {cellIdx, colName} ---
    % Use exact column names from the timetables (case-sensitive, as in file)
    ...
    % --- Step 2: Extract each wanted signal ---
    for i = 1:numel(signals)
        ...
        if isKey(lookup, signame)
            ...
            try
                tt    = Data{ci};
                % Use RowTimes — works regardless of time column name ('t', 'Time', etc.)
                t_raw = tt.Properties.RowTimes;
                % Convert duration to seconds (relative to start)
                ...
                % Flatten multi-column if needed
    ...
    if isfield(S, fn_test)
        ...
        % Show what matlab2fieldname returns
        fn2 = regexprep('speed_can', '[^a-zA-Z0-9_]', '_');
        fprintf('  [DEBUG] matlab2fieldname gibt: "%s"\n', fn2);
    end

    % --- Common time base from first valid signal ---
    S.t_base    = [];
    S.t_base_ok = false;
    for i = 1:numel(signals)
...
function fname = matlab2fieldname(signame)
    % Convert CAN signal name to valid MATLAB struct field name
...
function S = computeDerived(S)
    % Wheel angular velocities (rpm -> rad/s)
    % P_mech = Tq_ist * omega
    wheels = {'fl','fr','rl','rr'};
    ...
    % Total mechanical power (sum of 4 wheels)
    ...
    % ── GPS Track via double integration of INS_acc_x/y ──────────────────
    % Step 1: try velocity signals first (more accurate, single integration)
    % Step 2: fall back to acceleration (double integration + highpass)
    S.gps_x_ok = false;
    ...
    if ~S.gps_x_ok && isfield(S,ax_fn) && S.([ax_fn '_ok']) && ...
        ...
        try
            ...
            % Highpass filter to remove sensor bias/drift (cutoff ~0.02 Hz)
            % Simple 1st-order IIR highpass: y[n] = alpha*(y[n-1] + x[n] - x[n-1])
            alpha = 0.995;
            ...
            % Highpass again on velocity to remove integration drift
...
function buildGUI(S, meta, filepath)
    ...
    C.missing   = [0.50 0.50 0.50]; % color for missing data labels
    ...
    % --- Top Bar ---
    buildTopBar(fig, meta, filepath, S, C);

    % --- Time Window Slider Panel ---
    sliderH = 52;
    sliderPanel = uipanel(fig, 'Position', [0 900 1600 sliderH], ...
        'BackgroundColor', [0.10 0.10 0.10], 'BorderType', 'none');

    uilabel(sliderPanel, 'Position', [8 17 55 18], ...
        'Text', 't start (s)', 'FontSize', 9, 'FontColor', C.red, ...
        'BackgroundColor', 'none');
    efStart = uieditfield(sliderPanel, 'numeric', ...
        'Position', [68 15 65 22], 'Value', 0, ...
        'Limits', [0 t_total], 'FontSize', 9, ...
        'BackgroundColor', C.card, 'FontColor', C.white);
    slStart = uislider(sliderPanel, ...
        'Position', [145 28 550 3], ...
        'Limits', [0 t_total], 'Value', 0, ...
        'MajorTicks', [], 'MinorTicks', []);

    uilabel(sliderPanel, 'Position', [720 17 55 18], ...
        'Text', 't end (s)', 'FontSize', 9, 'FontColor', C.red, ...
        'BackgroundColor', 'none');
    efEnd = uieditfield(sliderPanel, 'numeric', ...
        'Position', [780 15 65 22], 'Value', t_total, ...
        'Limits', [0 t_total], 'FontSize', 9, ...
        'BackgroundColor', C.card, 'FontColor', C.white);
    slEnd = uislider(sliderPanel, ...
        'Position', [858 28 550 3], ...
        'Limits', [0 t_total], 'Value', t_total, ...
        'MajorTicks', [], 'MinorTicks', []);
    ...
    % --- Tab Group (leave room for slider) ---
    tg = uitabgroup(fig, 'Position', [0 0 1600 900], ...
        'TabLocation', 'top');
...
[valLabels, sigInfo] = buildTabAllSignals(tabs(9), S, C);
    ...
    % --- Collect all axes for time-window refresh ---
    ...
    function applyTimeWindow(ts, te)
        ...
        % Apply xlim to all uiaxes in figure
...
function btnSnap = buildTopBar(fig, meta, filepath, S, C)
    ...
    uilabel(topbar, 'Position', [430 10 380 25], ...
        'Text', [fn ext], ...
        'FontSize', 10, 'FontColor', C.gray2, 'BackgroundColor', 'none');
    ...
    % Callback noch LEER — wird in buildGUI gesetzt
    btnSnap = uibutton(topbar, 'Position', [1020 8 115 28], ...
        'Text', '▶  Snapshot', ...
        'BackgroundColor', C.red, 'FontColor', C.white, 'FontWeight', 'bold');

    % efSnap im UserData des Buttons speichern damit buildGUI drauf zugreifen kann
...
function buildTabDashboard(tab, S, meta, C)
    % --- Left column: KPIs + Status + Speed plot ---
    leftW = 780;
...
function buildTabBattery(tab, S, C)
    ...
    plotOrMissing(ax1, S, {'ams_overall_voltage_can','IVT_Result_I_can'}, ...
        {'V pack (V)','I (A)'}, C);
...
function buildTabPowertrain(tab, S, C)
    ...
    for i = 1:4
        w = wheels{i};
        buildInverterCard(tab, S, w, labels{i}, C, [xpos(i) 730 375 175]);
    end
...
function buildTabEfficiency(tab, S, C)
    ...
    if isfield(S,'E_total_ok') && S.E_total_ok && isfield(S,'E_regen_ok') && S.E_regen_ok
        e_net_v = S.E_total_kWh - S.E_regen_kWh;
        e_net   = sprintf('%.2f', e_net_v);
    ...
    % Powertrain efficiency over time
    ...
    % Pack power vs mechanical power
...
function buildTabTemperatures(tab, S, C)
    ...
    % Temp warning/critical indicators
...
function buildTabSlipTC(tab, S, C)
    ...
    plotOrMissing(ax1, S, {'slip_compare_val_fl_can','slip_compare_val_fr_can',...
        ...
        {'Slip FL','Slip FR','Slip RL','Slip RR'}, C);

    % TC slip target + strength
    ax2 = buildPlotArea(tab, [800 600 770 230], 'TC Eingriff & Slip Target', C);
    plotOrMissing(ax2, S, {'tqv_status_tc_slip_target_can','tqv_status_tqv_strength_can','tqv_status_tqv_base_strength_can'}, ...
        {'Slip Target','TQV Strength','Base Strength'}, C);
...
function buildTabPDU(tab, S, C)
    ...
    plotOrMissing(ax3, S, {'PDU_eFuse_ACE_IMON_can','PDU_eFuse_AMS_IMON_can',...
        'PDU_eFuse_DVSC_IMON_can','PDU_eFuse_SDC_IMON_can'}, ...
        {'ACE','AMS','DVSC','SDC'}, C);

    % Inverter eFuse currents
    ...
    plotOrMissing(ax4, S, {'PDU_eFuse_Inv_FL_IMON_can','PDU_eFuse_Inv_FR_IMON_can',...
        'PDU_eFuse_Inv_RL_IMON_can','PDU_eFuse_Inv_RR_IMON_can'}, ...
        {'Inv FL','Inv FR','Inv RL','Inv RR'}, C);

    % SDC state
    ax5 = buildPlotArea(tab, [800 150 770 210], 'SDC & Safety', C);
    plotOrMissing(ax5, S, {'SDC_AS_closed_b_can','SDC_Latch_Ready_b_can',...
        'imd_ok_b_can','ams_ok_b_can'}, ...
...
function plotOrMissing(ax, S, signames, labels, C, isDerived)
    ...
    for i = 1:numel(signames)
        ...
        % Handle derived signals (struct fields) vs CAN signals
        if isDerived
            fn = sn; % already a field name
        else
            fn = matlab2fieldname(sn);
        end
        ...
        if isfield(S, fn) && isfield(S, [fn '_ok']) && S.([fn '_ok'])
            try
                ...
                if isa(ts, 'timeseries') && ~isempty(ts.Data)
                    ...
                    plot(ax, tVec, dVec, 'Color', col, 'LineWidth', 1.2, 'DisplayName', lbl);
                    nPlotted = nPlotted + 1;
    ...
    % Show missing signal indicators
    if ~isempty(missingLabels)
        missingStr = sprintf('Keine Daten: %s', strjoin(missingLabels, ', '));
        ylims = ylim(ax);
        xlims = xlim(ax);
        if nPlotted == 0
            % Full missing — fill area
            ...
            % Partial missing — footnote
            text(ax, xlims(1), ylims(1), missingStr, ...
...
function plotGPS(ax, S, C)
    if ~isfield(S,'gps_x_ok') || ~S.gps_x_ok
        markMissing(ax, 'GPS / INS Daten nicht verfügbar', C);
        return;
    end
...
function plotGG(ax, S, C)
    ...
    if ~isfield(S,fn_ax) || ~S.([fn_ax '_ok']) || ...
       ~isfield(S,fn_ay) || ~S.([fn_ay '_ok'])
        markMissing(ax, 'IMU Daten (INS_acc_x/y) nicht verfügbar', C);
        return;
    end
    ...
    try
        if isfield(S,fn_sp) && S.([fn_sp '_ok'])
            ...
            colormap(ax, 'hot');
            cb = colorbar(ax); cb.Color = C.gray;
            cb.Label.String = 'Speed (km/h)';
        else
...
function plotCellHeatmap(ax, S, mode, C)
    ...
    for i = 1:nCells
        r = floor((i-1)/nCols) + 1;
        c = mod(i-1,nCols) + 1;
        sn = sprintf('%s%03d_can', prefix, i);
        fn = matlab2fieldname(sn);
        if isfield(S,fn) && isfield(S,[fn '_ok']) && S.([fn '_ok'])
            try
                ts = S.(fn);
                if isa(ts,'timeseries')
                    data(r,c) = mean(ts.Data,'omitnan');
                else
                    data(r,c) = mean(ts,'omitnan');
                end
                nFound = nFound + 1;
            catch; end
        end
    ...
    if nFound == 0
        markMissing(ax, sprintf('Keine Zell-%s Daten', unit), C);
        return;
    end
    ...
    axis(ax,'tight');
    xlabel(ax, sprintf('Spalte  (%d/%d Zellen gefunden)', nFound, nCells), ...
        'Color', C.gray, 'FontSize', 8);
    ylabel(ax, 'Reihe', 'Color', C.gray, 'FontSize', 8);
end

function plotEnergyPerLap(ax, S, C)
    % Estimate energy per lap from lap counter
    fn_lap = matlab2fieldname('lap_cnt_can');
    if ~isfield(S,'P_pack_ok') || ~S.P_pack_ok || ...
       ~isfield(S,fn_lap) || ~S.([fn_lap '_ok'])
        markMissing(ax, 'Lap counter oder Pack-Leistung nicht verfügbar', C);
        return;
    end
    ...
    try
        ...
        markMissing(ax, sprintf('Berechnung fehlgeschlagen: %s', ME.message), C);
    end
end

function plotRegenVsBrake(ax, S, C)
    if ~isfield(S,'P_pack_ok') || ~S.P_pack_ok
        markMissing(ax, 'Pack-Leistung nicht verfügbar', C);
        return;
    end
    try
        t  = S.P_pack.Time;
        P  = S.P_pack.Data;
        P_regen = -P .* (P < 0);  % positive when regenerating
        P_drive =  P .* (P > 0);

        plot(ax, t, P_drive/1000,  'Color', C.red,    'LineWidth', 1.0, 'DisplayName', 'Antrieb (kW)');
        hold(ax,'on');
        plot(ax, t, P_regen/1000, 'Color', C.green,  'LineWidth', 1.0, 'DisplayName', 'Rekuperation (kW)');
        hold(ax,'off');
...
% =========================================================
%% TAB 9 — ALLE SIGNALE  (kompakte 4-Spalten-Tabelle)
% =========================================================
function [valLabels, sigInfo] = buildTabAllSignals(tab, S, C)
    ...
    % Zeitstempel-Label oben links im Tab
    lblTime = uilabel(tab, ...
        'Position',  [6 TAB_H-16 400 14], ...
        'Text',      '● letzter Wert  —  Snapshot-Zeit über Topbar setzen', ...
        'FontSize',  7.5, 'FontColor', [0.45 0.45 0.45], ...
        'BackgroundColor', 'none');

    valLabels = gobjects(0);   % wächst dynamisch
    sigInfo   = {};            % {fn, ok_fn} pro Label

    for ci = 1:N_COLS
        x0   = 2 + (ci-1)*colW;
        yTop = TAB_H - 20;     % unter dem Zeit-Label starten
        ...
        for ei = idxFrom:idxTo
            ...
            [v, fc] = getSignalValAt(S, fn, ok_fn, []);  % [] = letzter Wert
    ...
    % lblTime ebenfalls speichern (Index 1 im sigInfo als Sonder-Eintrag)
...
% =========================================================
%% FLACHE SIGNALLISTE (kategorisiert, ohne per-Zell-Kanäle)
% =========================================================
function entries = buildFlatSignalList()
    ...
    % ── ANTRIEB — FIX: kein trailing comma ───────────────
    ...
    % ── ABGELEITET — FIX: einzeln mit e{end+1} statt [e, {{...}}] ───────
    e{end+1} = struct('header','Abgeleitet (letzte Werte)');
...
function buildKPIRow(parent, kpiData, C, pos)
    % kpiData: cell array {label, value, unit}
    n   = size(kpiData,1);
    w   = floor(pos(3)/n) - 5;
    for i = 1:n
        xp = pos(1) + (i-1)*(w+5);
        buildKPICard(parent, kpiData{i,1}, kpiData{i,2}, kpiData{i,3}, C, [xp pos(2) w pos(4)]);
    end
end
...
function buildKPIRowCustom(parent, kpis, C, pos)
    ...
    for i = 1:n
        xp = pos(1) + (i-1)*(w+5);
        buildKPICard(parent, kpis{i,1}, kpis{i,2}, kpis{i,3}, C, [xp pos(2) w pos(4)]);
    end
end

function buildKPICard(parent, label, value, unit, C, pos)
    pnl = uipanel(parent, 'Position', pos, ...
        'BackgroundColor', C.card, 'BorderType','none');

    uilabel(pnl,'Position',[8 pos(4)-22 pos(3)-16 14],...
        'Text', upper(label), 'FontSize', 9, 'FontWeight','normal',...
        'FontColor', C.gray, 'BackgroundColor','none');
    ...
    uilabel(pnl,'Position',[8 8 pos(3)-16 pos(4)-32],...
        'Text', dispStr, 'FontSize', 18, 'FontWeight','bold',...
        'FontColor', C.white, 'BackgroundColor','none',...
...
function buildInverterCard(parent, S, wheel, label, C, pos)
    pnl = uipanel(parent,'Position',pos,...
        ...
        'HighlightColor',C.red,'BorderWidth',1);

    uilabel(pnl,'Position',[8 pos(4)-22 pos(3)-16 16],...
        'Text',label,'FontSize',9,'FontWeight','bold',...
        'FontColor',C.red,'BackgroundColor','none');

    fields = {
        ['unitek_' wheel '_motor_temp_can'],  'T mot';
        ['unitek_' wheel '_igbt_temp_can'],   'T IGBT';
        ['unitek_' wheel '_speed_motor_ist_can'], 'Speed';
        ['unitek_' wheel '_torque_motor_ist_can'],'Tq ist';
        ['unitek_' wheel '_Vdc_Bus_can'],     'Vdc';
        ['unitek_' wheel '_i_ist_can'],       'I ist';
    };
    units_list = {'°C','°C','rpm','Nm','V','A'};
    ...
    for i = 1:size(fields,1)
        ...
        if isfield(S,fn) && isfield(S,[fn '_ok']) && S.([fn '_ok'])
            try
                ts = S.(fn);
                if isa(ts,'timeseries'); v = ts.Data(end); else; v = ts(end); end
                vstr = sprintf('%.1f %s', v, units_list{i});
                col  = C.white;
            catch
                vstr = '--'; col = C.missing;
            end
        else
            vstr = 'N/A'; col = C.missing;
        end

        uilabel(pnl,'Position',[8 ystart-i*18 80 16],...
            'Text',fields{i,2},'FontSize',8,'FontColor',C.gray,'BackgroundColor','none');
        uilabel(pnl,'Position',[90 ystart-i*18 pos(3)-100 16],...
...
function buildWheelPowerSummary(parent, S, C, pos)
    ...
    uilabel(pnl,'Position',[0 pos(4)-20 pos(3) 16],...
        'Text','Mittlere Radleistung','FontSize',10,'FontWeight','bold',...
        'FontColor',C.red,'BackgroundColor','none');
    ...
    for i = 1:4
        w = wheels{i};
        if isfield(S,['P_mech_' w '_ok']) && S.(['P_mech_' w '_ok'])
            try
                vals(i) = mean(abs(S.(['P_mech_' w]).Data)) / 1000; % kW
                found = true;
            catch; end
        end
    end
    ...
    if found
        ...
        b.CData = repmat(C.red,4,1);
        set(ax,'XTickLabel',xlbls,'XTick',1:4);
...
function buildStatusBadges(parent, S, C, pos)
    checks = {
        'ams_ok_b_can',     'AMS OK';
        'imd_ok_b_can',     'IMD OK';
        'SDC_AS_closed_b_can','SDC closed';
        'DL_EBS_state_can', 'EBS';
    };
    xp = pos(1);
    for i = 1:size(checks,1)
        ...
        pnl2 = uipanel(parent,'Position',[xp pos(2) 130 30],...
            'BackgroundColor',bgc,'BorderType','none');
        uilabel(pnl2,'Position',[5 5 120 20],'Text',lbl,...
...
function buildLapTable(parent, S, C, pos)
    ...
    pnl = uipanel(parent,'Position',pos,...
        'BackgroundColor',C.panel,'BorderType','none');
    uilabel(pnl,'Position',[0 pos(4)-20 pos(3) 16],...
        'Text','Lap Übersicht','FontSize',10,'FontWeight','bold',...
        'FontColor',C.red,'BackgroundColor','none');
    ...
    % Build lap summary table
    try
        lap_ts = S.(fn_lap);
        laps = round(lap_ts.Data);
        ulaps = unique(laps); ulaps = ulaps(ulaps > 0);
        nLaps = min(numel(ulaps), 8); % max 8 rows to fit
        ...
        for i = 1:nLaps
            ...
            mask = laps == ulaps(i);
            bgc  = C.card;
            if mod(i,2)==0; bgc = C.panel; end
            bk = uipanel(pnl,'Position',[0 yp pos(3) 20],...
                'BackgroundColor',bgc,'BorderType','none');
            ...
            if isfield(S,fn_spd) && S.([fn_spd '_ok'])
                try
                    spd = S.(fn_spd);
                    spd_lap = interp1(spd.Time, spd.Data, lap_ts.Time(mask),'linear','extrap');
                    spd_str = sprintf('%.0f km/h', mean(spd_lap,'omitnan'));
                catch; end
            end
            ...
            if isfield(S,fn_soc) && S.([fn_soc '_ok'])
                try
                    soc = S.(fn_soc);
                    soc_lap = interp1(soc.Time, soc.Data, lap_ts.Time(mask),'linear','extrap');
                    soc_str = sprintf('%.0f%%', soc_lap(end));
                catch; end
            end
...
function buildAMSErrors(parent, S, C, pos)
    ...
    pnl = uipanel(parent,'Position',pos,...
        'BackgroundColor',C.panel,'BorderType','none');
    uilabel(pnl,'Position',[0 pos(4)-18 200 16],...
        'Text','AMS Fehler & Status','FontSize',10,'FontWeight','bold',...
        'FontColor',C.red,'BackgroundColor','none');
    ...
    for i = 1:size(checks,1)
        ...
        if isfield(S,fn) && isfield(S,[fn '_ok']) && S.([fn '_ok'])
            ...
            bgc = [0.15 0.15 0.15]; fc = C.missing; lbl = [lbl ' (N/A)'];
        end
        p2 = uipanel(pnl,'Position',[xp 5 140 35],...
            'BackgroundColor',bgc,'BorderType','none');
...
function buildTempStatusRow(parent, S, C, pos)
    % Show current peak temperatures for key systems
    checks = {
    ...
    pnl = uipanel(parent,'Position',pos,...
        'BackgroundColor',C.panel,'BorderType','none');
    uilabel(pnl,'Position',[0 pos(4)-18 300 16],...
        'Text','Temperatur Status — Spitzenwerte','FontSize',10,...
        'FontWeight','bold','FontColor',C.red,'BackgroundColor','none');
    ...
    for i = 1:size(checks,1)
        ...
        if isfield(S,fn) && isfield(S,[fn '_ok']) && S.([fn '_ok'])
            try
                ts   = S.(fn);
                vmax = max(ts.Data);
                valStr = sprintf('%.0f°C', vmax);
                if vmax >= crit;      fc = C.red;    bgc = [0.3 0.05 0.05];
                elseif vmax >= warn;  fc = C.orange; bgc = [0.25 0.15 0.0];
                else;                 fc = C.green;  bgc = [0.05 0.2 0.05];
                end
            catch
                valStr = 'ERR'; fc = C.missing; bgc = [0.15 0.15 0.15];
            end
        else
            valStr = 'N/A'; fc = C.missing; bgc = [0.15 0.15 0.15];
        end

        p2 = uipanel(pnl,'Position',[xp 5 175 35],'BackgroundColor',bgc,'BorderType','none');
        uilabel(p2,'Position',[5 18 100 14],'Text',lbl,...
            'FontSize',8,'FontColor',C.gray,'BackgroundColor','none');
        uilabel(p2,'Position',[5 3 100 16],'Text',valStr,...
            'FontSize',11,'FontWeight','bold','FontColor',fc,'BackgroundColor','none');
        xp = xp + 182;
    end
end
...
function buildDCDCCard(parent, S, name, C, pos)
    base  = 'PDU_DCDC_';
    fn_v  = matlab2fieldname([base name '_vout_can']);
    fn_i  = matlab2fieldname([base name '_iout_can']);
    fn_t  = matlab2fieldname([base name '_temp_can']);
    fn_e  = matlab2fieldname([base name '_e_b_can']);

    pnl = uipanel(parent,'Position',pos,...
        'BackgroundColor',C.card,'BorderType','line','HighlightColor',C.gray2);
    ...
    for j = 1:3
        ...
        if isfield(S,fn) && isfield(S,[fn '_ok']) && S.([fn '_ok'])
            try; v = S.(fn).Data(end); vstr = sprintf('%.1f%s',v,unit);
            catch; vstr = ['--' unit]; end
            fc = C.white;
        else
            vstr = ['N/A ' unit]; fc = C.missing;
        end
        uilabel(pnl,'Position',[5 yp-j*16 pos(3)-10 14],...
    ...
    uilabel(pnl,'Position',[5 5 pos(3)-10 14],...
        'Text', ternStr(hasErr,'FEHLER','OK'),...
...
function buildEFusePanel(parent, S, C, pos)
    pnl = uipanel(parent,'Position',pos,...
        'BackgroundColor',C.panel,'BorderType','none');
    uilabel(pnl,'Position',[0 pos(4)-18 200 16],...
        'Text','eFuse Monitor','FontSize',10,'FontWeight','bold',...
        'FontColor',C.red,'BackgroundColor','none');
    ...
    for h = 1:4
        uilabel(pnl,'Position',[5+(h-1)*colW pos(4)-36 colW-2 16],...
            'Text',headers{h},'FontSize',8,'FontWeight','bold',...
            'FontColor',C.red,'BackgroundColor','none');
    end
    ...
    for i = 1:nShow
        ...
        bgc  = (mod(i,2)==0)*C.card + (mod(i,2)~=0)*C.panel;
        ...
        uilabel(bk,'Position',[5+colW 2 colW-2 15],'Text',istr,...
        ...
        if isfield(S,fn_e) && isfield(S,[fn_e '_ok']) && S.([fn_e '_ok'])
            ...
            estr = ternStr(hasErr,'JA','nein');
            efc  = hasErr*C.red + (~hasErr)*C.green;
        else; estr='N/A'; efc=C.missing; end
        uilabel(bk,'Position',[5+2*colW 2 colW-2 15],'Text',estr,...
...
function kpiData = computeKPIs(S)
    fn_spd  = matlab2fieldname('speed_can');
    fn_soc  = matlab2fieldname('ams_capacity_fl_can');
    fn_tmot = matlab2fieldname('unitek_rl_motor_temp_can');
    fn_tbat = matlab2fieldname('ams_cell_max_temp_can');
...
function v = getSignalMax(S, signame, def)
    ...
    if isfield(S,fn) && isfield(S,[fn '_ok']) && S.([fn '_ok'])
        try
            ts = S.(fn);
            if isa(ts,'timeseries'); v = max(ts.Data); else; v = max(ts); end
            v = sprintf('%.1f',v);
        catch; v = def; end
    else; v = def; end
end
    ...
    if isfield(S,fok) && S.(fok)
        try
            ...
            if strcmp(mode,'mean_data') && isa(raw,'timeseries')
                d = raw.Data(raw.Data > 5);
                v = sprintf('%.1f', mean(d,'omitnan'));
            elseif strcmp(mode,'mean_data') && isnumeric(raw)
                v = sprintf('%.1f', raw);
            else
                if isa(raw,'timeseries'); v = sprintf('%.2f',raw.Data(end));
                else; v = sprintf('%.2f',raw); end
            end
        catch; v = def; end
    else; v = def; end
end
...
function v = getLastBool(S, signame)
    ...
    if isfield(S,fn) && isfield(S,[fn '_ok']) && S.([fn '_ok'])
        try
            ts = S.(fn);
            if isa(ts,'timeseries'); bv = ts.Data(end) >= 1;
            else; bv = ts(end) >= 1; end
...
function markMissing(ax, msg, C)
    ...
    text(ax, 0.5, 0.55, 'KEINE DATEN', ...
        'Color', C.missing, 'FontSize', 11, 'FontWeight', 'bold', ...
        'HorizontalAlignment','center','VerticalAlignment','middle');
    text(ax, 0.5, 0.38, msg, ...
        'Color', [0.45 0.45 0.45], 'FontSize', 8, ...
        'HorizontalAlignment','center','VerticalAlignment','middle');
    ax.XTick = []; ax.YTick = [];
end

% Wert eines Signals zum Zeitpunkt t_snap ([] = letzter Wert)
...
% Tab-Werte auf neuen Zeitstempel aktualisieren
...
function exportReport(S, meta)
    [fname, fpath] = uiputfile({'*.pdf','PDF Report';'*.png','PNG Screenshot'},...
        'Report speichern', ...
        sprintf('Report_%s_%s', meta.vehicle, strrep(meta.date,'-','')));
...
function signals = getSignalList()
    % Complete list of all expected CAN signals
    signals = {
    ...
    % Add cell voltage channels (144)
    for i = 1:144
        signals{end+1} = sprintf('ams_cell_voltage%03d_can', i);
    end
    % Add cell temperature channels (48)
    for i = 1:48
        signals{end+1} = sprintf('ams_cell_temp%03d_can', i);
    end
end
...
function buildSnapshotWindow(S, t_snap, C)
    ...
    % --- Filterfeld ---
    uilabel(fig2, 'Position', [14 742 45 20], ...
        'Text', 'Filter:', 'FontSize', 9, ...
        'FontColor', C.gray, 'BackgroundColor', 'none');
    efFilter = uieditfield(fig2, 'text', ...
        'Position', [62 740 430 24], ...
        'FontSize', 9, 'BackgroundColor', C.card, 'FontColor', C.white, ...
        'Placeholder', 'Signalname filtern...');

    lblCount = uilabel(fig2, 'Position', [504 742 300 22], ...
        'Text', '...', 'FontSize', 9, 'FontColor', C.gray, ...
        'BackgroundColor', 'none', 'HorizontalAlignment', 'right');

    % --- Alle Signale @ t_snap sammeln ---
    allNames = {};
    allVals  = {};
    ...
    for i = 1:numel(fields)
        ...
        if any(strcmp(fn, {'t_base','t_base_ok','gps_x','gps_y',...
                           'gps_t','E_total_kWh','E_regen_kWh'}));   continue; end

        ok_fn = [fn '_ok'];

        % Skalare Ableitungsgrößen (z. B. E_total_kWh)
        if ~isfield(S, ok_fn) && isnumeric(S.(fn)) && isscalar(S.(fn))
            allNames{end+1} = fn;
            allVals{end+1}  = sprintf('%.5g  [konstant]', S.(fn));
            continue;
        end
        ...
        try
            ...
            else;  val = interp1(tVec, dVec, t_snap, 'linear');
            end

            sigLabel = ts.Name;
            if isempty(sigLabel); sigLabel = fn; end

            allNames{end+1} = sigLabel;
            allVals{end+1}  = sprintf('%.5g', val);
        catch
        end
    end
    ...
    if nSigs == 0
        uilabel(fig2, 'Position', [14 380 790 30], ...
            'Text', 'Keine Signaldaten im angegebenen Zeitraum verfügbar.', ...
            'FontSize', 12, 'FontColor', C.missing, 'BackgroundColor', 'none', ...
            'HorizontalAlignment', 'center');
        return;
    end

    % --- Tabelle aufbauen ---
    colHeader = sprintf('Wert  @  t = %.3f s', t_snap);
    uit = uitable(fig2, ...
        'Position',     [14 14 790 718], ...
        'Data',         [allNames(:), allVals(:)], ...
        'ColumnName',   {'Signal', colHeader}, ...
        'ColumnWidth',  {460, 250}, ...
        'FontSize',     10, ...
        'RowName',      {}, ...
        'BackgroundColor', [C.card; C.panel], ...
        'ForegroundColor',  C.white);

    % --- Live-Filter Callback ---
...
function filterSnapshotTable(uit, allNames, allVals, filterStr, lblCount, nTotal)
    ...
    if isempty(fs)
        ...
        mask = contains(lower(allNames(:)), lower(fs));
    end
